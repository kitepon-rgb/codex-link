...
- `src/websocket.ts`: Host / iPhone placeholder WebSocket gateway、Host bootstrap HTTP API、device session pairing / revocation、device credential rotation、HostAccess sharing API。
...
- `CODEX_LINK_DEVICE_CREDENTIAL_TTL_MS`: device credential の有効期間。未設定時は `2592000000`、最小値は `60000`。
...
response には `userId`、`deviceId`、`deviceToken`、`deviceTokenExpiresAt`、`hostId` が含まれます。Relay は `deviceToken` 本体を保存せず、SHA-256 hash と expiry だけを保存します。Host は以後の Relay WebSocket 接続で `Authorization: Bearer <deviceToken>` を送ります。
...
response には `userId`、`deviceId`、`deviceToken`、`deviceTokenExpiresAt` が含まれます。iPhone app はこの `deviceToken` を保存し、Relay WebSocket、pairing、revocation API で `Authorization: Bearer <deviceToken>` を送ります。
...
device credential は `POST /api/device-credential/rotate` に現在の `Authorization: Bearer <deviceToken>` を付けると rotate できます。古い token は即時に無効になり、response の `deviceTokenExpiresAt` が新しい有効期限です。
...
```json
{
  ...
  "deviceTokenExpiresAt": "2026-06-10T00:00:00.000Z"
}
```
...
これは production authentication ではありません。現時点の credential は TTL / rotation 付きの device ごとの bearer token で、外部 IdP、短命 user session、永続 storage はまだありません。
