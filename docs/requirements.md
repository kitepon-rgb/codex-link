...
- Host から作った thread が Codex CLI 履歴に出るか。2026-05-11 検証 (`docs/mvp-plan.md` の「cross-source thread 可視性検証」) で、rollout は `~/.codex/sessions/` に正しく書かれ、`session_meta.source` は `"vscode"` として記録されることを確認 (Codex CLI 0.130.0 は app-server 経由でも source を `vscode` 扱いする)。`thread/list` の default filter にも含まれることを `scripts/codex-app-server-thread-source-probe.mjs` で実測。CLI 側 picker からも見える前提で扱える。
- Host から作った thread が VS Code Codex 履歴に出るか。上記と同じく source `"vscode"` で記録されるため、VS Code 拡張の default 一覧 filter に乗る前提。実 VS Code + Codex 拡張での目視確認は別途必要 (コード smoke では完結しない)。
