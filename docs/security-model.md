...
MVP の rate limit は単一 Relay process 内の in-memory window です。device session creation / pairing / revoke、HostAccess grant / revoke、Host bootstrap、Host pairing code creation、client subscribe / route を対象にします。HTTP JSON body は `CODEX_LINK_MAX_HTTP_BODY_BYTES` で上限を設定し、超過時は `PAYLOAD_TOO_LARGE` として拒否します。複数 process で共有される quota、永続化、user plan 別の制御は hardening phase の対象です。
